...
def retrieveProgramPage(program_url):
    ...
    try:
        if browser.find_element_by_xpath("//a[text()=\"I'm a domestic student\"]"):
            browser.find_element_by_xpath("//a[text()=\"I'm a domestic student\"]").click()
    except:
        print("You already on domestic student page!")
    try:
        ...
        duration = htmlPage('.program__table').find("[data-sinet='StudentInfo > Domestic > PROGRAM_DURATION']").text()
        ...
        cur.execute('''INSERT into domestic_Program (title, overview, program_code, QTAC_code, faculty, duration, commencing, program_level, unit, delivery_location, AQF, entry_requirement, major, fee, course)
                                        values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)''', (
            title, description, program_code, QTAC_code, program_faculty, duration, commencing, program_level, units, delivery_location, AQF, entry_requirement, majors, fee, courses))
        conn.commit()
    except TimeoutException:
        print('No such program for domestic student')
